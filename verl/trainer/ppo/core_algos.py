...
def compute_grpo_outcome_advantage(
    ...
    Returns:
        ...
        reward_variance: `(float)`
            Average variance of rewards across all prompts
        reward_mean: `(float)`
            Mean of outcome rewards over the batch
    """
    ...
    with torch.no_grad():
        ...
        # Calculate reward variance across all prompts
        variances = []
        for idx in id2score:
            if len(id2score[idx]) == 1:
                id2mean[idx] = torch.tensor(0.0)
                id2std[idx] = torch.tensor(1.0)
                variances.append(0.0)  # No variance for single sample
            elif len(id2score[idx]) > 1:
                id2mean[idx] = torch.mean(torch.tensor(id2score[idx]))
                id2std[idx] = torch.std(torch.tensor([id2score[idx]]))
                variance = torch.var(torch.tensor(id2score[idx]))
                variances.append(variance.item())
            else:
                raise ValueError(f"no score in prompt index: {idx}")
        
        # Average variance across all prompts
        reward_variance = np.mean(variances) if variances else 0.0
        
        # Batch mean reward (outcome)
    ...
    return scores, scores, reward_variance, reward_mean

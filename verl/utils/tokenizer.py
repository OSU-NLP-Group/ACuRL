...
def hf_processor(name_or_path, **kwargs):
    ...
    # First try to load from huggingface subdirectory if it exists
    huggingface_path = os.path.join(name_or_path, "huggingface")
    if os.path.exists(huggingface_path):
        try:
            processor = AutoProcessor.from_pretrained(huggingface_path, **kwargs)
            print(f"Successfully loaded processor from huggingface directory: {huggingface_path}")
        except Exception as e:
            print(f"Failed to load processor from huggingface directory {huggingface_path}: {e}")
            processor = None
    
    # If huggingface directory doesn't exist or loading failed, try the original path
